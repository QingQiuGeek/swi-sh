# 首页 UI 精致化 方案（v1.0 已实施）

> 版本：v0.1（提案）→ **v1.0 已实施（第 7 轮）**
...
> 确认后已改代码，结论回写 `visual.md` §6 / `design.md` §2.2 §2.4 §4.2 / `avoid.md` §4 / `page-specs.md` §3.1 / `prd.md` §12.1（C22–C24）。
> **本文件自此只作为决策记录保留**：与代码不一致时，以 `design.md` 为准（实施偏差见 §12.3）。
...
3. 360px 宽度无横向滚动条；ScrollRail 与悬浮工具轨不出现。（**已作废**：悬浮工具轨后改为「任何宽度都显示，且始终在视口右侧垂直居中」，理由见 `design.md` §8 与 `ui-patterns.md` §8.7；本条保留为当时的决策记录。）
4. 切换 `zh` / `en` 后无残留文案。
...
| # | 结论 |
| --- | --- |
| D1 衬线字体 | **不引入** —— 全站标题继续 Inter + Noto Sans SC，`--font-heading` 与 `--font-sans` 同族 |
| D2「关于我们」底色 | **选 A1 暖米 `highlight-soft` `#FBF0E2`**（理由见 12.2） |
| D3 CTA 通栏 | **加** —— 位置在「关于我们」之后、「常见问题」之前，底色 `primary`，按钮滚到 `products` |

### 12.2 D2 为什么选暖米，而不是深蓝

- 深蓝底（方案 A2 / D）会与紧随其后的「立即投保」深蓝通栏**同底相邻**，两栏连成一条长深色带，正好踩中本文 §4.3 自己列的坑「相邻两栏不能选同一档底色」。用户要求通栏必须是深蓝（D3），所以「关于我们」只能让出深蓝。
- 前三栏已经全是冷色（248 / 241 / 232 三级压深），再加一档冷色（`secondary`）等于把「单调朴素」原样保留；暖米是唯一能当「换气口」的档位，而且正好接住方案 B 新引入的暖棕眉标。
- 代价是 `design.md` §2.2 的「暖色总占比 < 10%」必须放宽 —— 已改写为「暖色可用于分区底色与眉标，不用于正文与大面积按钮」，并在 §2.4 补了四档浅底上的实测对比度。

### 12.3 实施偏差（与 v0.1 方案不同之处）

| 项 | 方案 v0.1 | 实际实现 | 原因 |
| --- | --- | --- | --- |
| 投保案例底色 | `bg-secondary/65` | `bg-secondary` | `/65` 混出来是 `rgb(240,244,250)`，与 `bg-muted` 的 `rgb(241,245,250)` 只差 1 个色阶，加了发丝线也像同一栏 |
| 眉标色值 | 沿用 `--highlight` `#9E5E18` | 调深为 `#98590F`（`oklch(0.5250 …)`） | 原值在最深的 `secondary` 底上只有 4.43:1，不满足「任一 < 4.5:1 即不合格」 |
| FAQ「抬头居中」 | §6 表格里写了 | **未采纳**，保持问题左对齐 | §2.2 实测参考站是「问题左对齐」，§6 这半句与实测冲突，以实测为准 |
| `StepList` 序号 | 「是描边圆」→ 改实心 | 已是 `bg-primary` 实心圆，只补了列间竖线 | 现状诊断过时 |
| `ProductCard` 图标底衬 / 价格区发丝线 | 待补 | 已存在（`bg-secondary` + `Separator`），未改 | 同上 |
| `StatBlock` 格间竖线 | 「格间加发丝线」 | 已有 `border-l`；在暖米底上改用 `border-highlight/25` | 冷灰线压在暖底上会显脏 |
| CTA 通栏按钮高度 | 未指定 | 显式 `h-11`（44px） | 当前 shadcn 的 `Button size="lg"` 是 `h-9`，与 `design.md` §4.4 的 44px 不符；首屏 CTA 也是这么处理的 |

### 12.4 实测对比度（在 Chrome 里用 canvas 取真实渲染像素计算）

| 组合 | 对比度 |
| --- | --- |
| 眉标 `highlight` / `background` | 5.36 |
| 眉标 `highlight` / `muted` | 5.09 |
| 眉标 `highlight` / `secondary` | **4.77（最低）** |
| 眉标 `highlight` / `highlight-soft` | 4.95 |
| 大标题 `primary` / `background` | 10.97 |
| 正文 `muted-foreground` / `secondary` | **4.66（最低）** |
| 常见问题 `foreground` / `background` | 16.64 |
| CTA 通栏 `primary-foreground` / `primary` | 11.39 |

全部组合 ≥ 4.5:1；1440 与 390 两档宽度下 `document.scrollWidth === innerWidth`，无横向溢出。内容页 `/[locale]/products`、`/guide`、`/about` 的抬头同步换色后仍为 5.36 / 10.97，无回归。

### 12.5 验收方式

1. 桌面 1440 与移动 390 逐分区截图，肉眼确认相邻两栏底色可区分。
2. 每个新底色上重跑对比度实测（见 12.4）。
3. Header 仍是五个 Tab；新 CTA 通栏与「常见问题」都不参与分区导航。